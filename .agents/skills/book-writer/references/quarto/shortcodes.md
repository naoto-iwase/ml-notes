...
- Store shared notation and reusable components in `{lang}/{book}/_shared/*.qmd`, and load them with `{{< include _shared/notation.qmd >}}`.
- Because entries under `_shared/` begin with an underscore, Quarto automatically excludes them from render targets.
- Write include paths relative to the file containing the include.
