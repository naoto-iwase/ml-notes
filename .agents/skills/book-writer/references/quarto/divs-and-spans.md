...
- Write theorems, lemmas, corollaries, propositions, and definitions in theorem environments such as `::: {#thm-name}`.
- `_quarto.yml` already configures the Japanese output labels `定理` (theorem), `補題` (lemma), `系` (corollary), `命題` (proposition), `定義` (definition), `例` (example), and `演習` (exercise).
- The callout syntax `::: {.callout-note}` is also a type of div. See [callouts.md](callouts.md) for details.

Available theorem prefixes:
