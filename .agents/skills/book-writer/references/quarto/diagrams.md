...
Mermaid node labels support HTML-like formatting when enclosed in `["..."]`.
...
```
A["<b>Title</b><br/>Description"]
```

**HTML entity escaping** — required inside quoted labels:

| Character | Escape    | Example                              |
|-----------|-----------|--------------------------------------|
| `&`       | `&amp;`   | `["Topic &amp; Quality"]`            |
| `<`       | `&lt;`    | `["x &lt; 10"]` (non-tag context)    |
| `>`       | `&gt;`    | `["x &gt; 0"]` (non-tag context)     |

::: {.callout-warning}
`\n` does **not** produce a line break in Mermaid — always use `<br/>`.
Unescaped `&` will break the Mermaid parser or produce garbled text.
:::
