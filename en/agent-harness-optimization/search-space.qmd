...
GEPA uses natural-language feedback on execution results to revise prompts and retains the union of candidates that perform well on different validation instances [@agrawal2026gepa]. Because it does not select a single candidate based only on average score, it can preserve candidates that fix one failure but not another.
